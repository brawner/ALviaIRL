	...
	public void euclideanDistVsIterations(int maxIterations, int nRuns, String outputPath){
		...
		for (int maxi = 0; maxi < maxIterations; maxi++) {
			...
			for (int run = 0; run < nRuns; run++) {
				...
				Policy projPolicy = ApprenticeshipLearning.projectionMethod(this.domain, planner, featureFunctions, expertEpisodes, GAMMA, FEXP_EPSILON, maxi,
										featureWeightHistory, featureWeightScoreHistory);
				...
				if (featureWeightScoreHistory.size() > 0) {
					results[maxi][run] = featureWeightScoreHistory.get(featureWeightScoreHistory.size() -1)*(1-GAMMA);
						//euclideanDist(apprenticeFExp, expertFExp, true);
					System.out.println(featureWeightScoreHistory.size());
