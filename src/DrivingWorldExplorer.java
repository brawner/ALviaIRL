...
import burlap.oomdp.visualizer.Visualizer;
	 ...
	 *
	 */
public class DrivingWorldExplorer extends JFrame{
	 ...
	 */
	public DrivingWorldExplorer(Domain domain, Visualizer painter, State baseState){
	 ...
	 */
	public DrivingWorldExplorer(Domain domain, Visualizer painter, State baseState, int w, int h){
		this.init(domain, painter, baseState, w, h);
	}
	
	public DrivingWorldExplorer(Domain domain, Visualizer painter, State baseState, int w, int h, RewardFunction rf){
		this.init(domain, painter, baseState, w, h);
	...
	protected void init(Domain domain, Visualizer painter, State baseState, int w, int h){
		...
		this.numSteps = 0;
		this.actionTimer = new Timer(refreshInterval, this.getActionListener());
	...
	private ActionListener getActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			String mappedAction;
			synchronized(DrivingWorldExplorer.this.currentAction) {
				mappedAction = DrivingWorldExplorer.this.currentAction;		
			}
			DrivingWorldExplorer.this.resetCurrentAction();
			...
			if(action == null){
				...
				double reward = 0;
				if (DrivingWorldExplorer.this.rewardFunction != null) {
					reward = DrivingWorldExplorer.this.rewardFunction.reward(DrivingWorldExplorer.this.curState, null, null);
				}			
				DrivingWorldExplorer.this.currentEpisode.recordTransitionTo(DrivingWorldExplorer.this.curState, groundedAction, reward);
				numSteps++;
			}
			
			painter.updateState(curState);
			DrivingWorldExplorer.this.updatePropTextArea(curState);
	...
	public void addWaitAction(String action) {
		this.waitAction = action;
		this.currentAction = this.waitAction;
	}
	
	public void resetCurrentAction() {
		synchronized(this.currentAction) {
			this.currentAction = this.waitAction;
		}
	}
	...
	public List<EpisodeAnalysis> initGUIAndRecord(List<EpisodeAnalysis> recordedEpisodes) {
		...
		SpecialExplorerAction action = new SpecialExplorerAction() {
			...
			public State applySpecialAction(State curState) {
				synchronized(DrivingWorldExplorer.this) {
					DrivingWorldExplorer.this.recordedEpisodes.add(DrivingWorldExplorer.this.currentEpisode);
					DrivingWorldExplorer.this.isRecording = !DrivingWorldExplorer.this.isRecording;
				}
				return curState;
			}
		};
	...
	protected void handleKeyPressed(KeyEvent e){
		...
		if(mappedAction != null){
			this.currentAction = mappedAction;
			if (!this.actionTimer.isRunning()) {
